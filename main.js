...
//app.use(helmet()); //protect your app from some well-known web vulnerabilities with Helmet
process.env.NODE_ENV = 'production';

//Check Node App environment
...
app.post('/forgot', function(req, res, next) {
	var noSuchEmail = req.body.email  + " does not exist in our records";
    async.waterfall([
		function(done) {
		...
		function(token, done){
			...
			getUserEmail(req.body.email,function(err,data){
				...
				if (!user.email) {
					errType = "X";
					errMsg = noSuchEmail + ". Have you got the right email?";
					res.redirect('/forgot');
				}
	...
	if(email!=""){
		...
		var salutation = "<h3>Hi there " + fmtEmail + ", </h3>";
		var content1 = "<h4>You have requested a change of password.</h4>";
		var content2 = "<h4>Do NOT click if this action was not done by you.</h4>";
		var resetURL= "http://" + req.headers.host + "/reset/" + token; 
		var resetContent = "<h4> Click <a href=" + resetURL + ">here </a> if this action was done by you <h4>"
		...
		const request = mailjet.post("send")
			.request({
				...
				"Text-part":"",
				"Html-part":salutation + "<br>"  + content1 + "<br>" + content2 + "<br>" + resetContent ,
				"Recipients":[{"Email": fmtEmail}]
			})
			request
			.then(result => {
				errType = "0";
				errMsg = "An e-mail has been sent to " + fmtEmail + " with further instructions."
				tempErrStatus = errType;
				res.redirect('/forgot');
			})
			.catch(err => {
...
function storeProcessedData(date,vehicle,workTime,company){
	connection.query('INSERT IGNORE INTO processed_data SET `date`=?,`vehicle`=?,`work_time`=?,`company`=?', [date,vehicle,workTime,company], function (error, results, fields) {
		if (error){
			throw error;
		}
	});
...
function updateProcessedDataMove(moveTime,date,vehicle){
	connection.query('UPDATE IGNORE processed_data SET `move_time`=? WHERE `date`=? AND `vehicle`=?', [moveTime,date,vehicle], function (error, results, fields) {
		if (error){
			throw error;
		}
	});
...
function updateProcessedDataStop(stopTime,date,vehicle){
	connection.query('UPDATE IGNORE processed_data SET `unknown_stop`=? WHERE `date`=? AND `vehicle`=?', [stopTime,date,vehicle], function (error, results, fields) {
		if (error){
			throw error;
		}
	});
...
function updateProcessedDataService(serviceTime,date,vehicle){
	connection.query('UPDATE IGNORE processed_data SET `service_time`=? WHERE `date`=? AND `vehicle`=?', [serviceTime,date,vehicle], function (error, results, fields) {
		if (error){
			throw error;
		}
	});
...
function removeProcessedData(datetime1,datetime2,vehicle,company){
	var isDeleted = 0;
	return new Promise(function(resolve, reject) {
		console.log("Remove Processed Data Datetime 2: " +  datetime2);
		connection.query('DELETE FROM node WHERE `datetime` < ? AND `vehicle`=? AND `company`=?', [datetime2,vehicle,company], function (error, results, fields) {
			if (error) reject(error);
			if(results.affectedRows>0){
				isDeleted == 1;
				setTimeout(() => {
    ...
    connection.query('SELECT datetime,vehicle,company from node WHERE speed>0 ORDER BY id LIMIT 1',function (error, results, fields) {
		...
		wait(500);
			...
			fixData(dateStr,dateStrTmr,veh,company)
			   ...
			   .then(() => getDataforFixv2(dateStr,dateStrTmr,veh,company))
			   ...
			   .then((counterX) => getChartResults(counterX)) 
			   .catch((err) => setImmediate(() => { throw err; })); 
		}
	});
}

function getChartResults(counterX){
    var dataItem = [];
    var promiseCounter = 0;
    connection.query('SELECT datetime,vehicle,company from node WHERE speed>0 ORDER BY id LIMIT 1', function (error, results, fields) {
        if(results.length>0){
			...
			getWorkTime(dateStr,dateStrTmr,veh,company)
			   ...
			   .then((isDone) => {
				...
				.then(() => removeProcessedData(dateStr,dateStrTmr,veh,company))
				.then(delayProc(1000))
...
app.get('/checkLoginStatus/', function (req, res) {
	var companyItem = {"company": req.session.company};
	res.end(JSON.stringify(companyItem));
});

app.get('/removeErrorStatus/', function (req, res) {
	errMsg = "";
	errType = "";
	res.end();
});

//For tracking error statuses
var tempErrStatus= "";

app.get('/getErrorMsg/', function (req, res) {
		var errJSON = {errorType: tempErrStatus};
		var results = JSON.stringify(errJSON);
		tempErrStatus = "";
		res.end(results);
});
					...
					var fmt = 'YYYY-MM-DD hh:mm';
						...
						var stream = XLSX.stream.to_csv(worksheet);
							stream.pipe(fs.createWriteStream(output_file_name)).on('finish', function () {
								...
								console.log("number of replicate rows: " +  + total);
								if(total<1){
									var query = connection.query('LOAD DATA LOCAL INFILE ' + "'" + output_file_name + "'" + ' INTO TABLE `node` FIELDS TERMINATED BY ? ENCLOSED BY ? ESCAPED BY ? LINES TERMINATED BY ? IGNORE 6 LINES (@datetime, @lat, @lon, @roadname, @speed) SET `datetime` = STR_TO_DATE(@datetime,?),lat=@lat,lon=@lon,roadname=@roadname,speed=@speed,vehicle=?,company=?',[comma,enclose,escape,terminator,'%d/%m/%Y %H:%i',vehicleNum,company],function (error, results, fields) {
										if (error){
											...
											console.log("Session value = " +errMsg);
											console.log("Upload DateTime: " + new Date());
											res.redirect("/upload");
											res.finished = true;
											return;
										}
...
//Main bulk of the fixes the data to smoothen the gaps in the received data - accuracy to 1 minute
...
function removeArrayDups(processData){
	var unique_array = []
...
/* function getNonDuplicateArray(processData) {
	var removeDupsArr = [];
	for (i = 0; i < processData.length-1; i++) {
		var itemCurr = i;
		var itemNext = i+1;
		if (processData[i].datetime != processData[i+1].datetime) {
			removeDupsArr.push(processData[i]);
		}
	}
	console.log("NondupARR SIZE: " +removeDupsArr.length);
...
function processFixDatav2(data,startDate,endDate,vehicleNum,company){
	var insertQuery = "INSERT IGNORE INTO `node` (datetime,lat,lon,roadname,speed,vehicle,company) VALUES ?";
	var counterX = 0;
	return new Promise(function(resolve, reject){
		if(data.length){
			connection.query(insertQuery,[data], function (error, results, fields) {
				if (error) throw (error);
				if(results.affectedRows>0){
					counterX = results.affectedRows;
					console.log("AFFECTED ROWS pfdv2: " +results.affectedRows);
					setTimeout(() => {
						console.log('promise - Delete Data');
						resolve(counterX);
					}, 1500);
...
app.post('/verifyLogin', function (req, res) {
	...
	errType = req.session.errType;
	connection.query('SELECT * FROM user WHERE `username`=? OR `email`=?' , [username,username], function (error, results, fields) {
		if (error) throw error;
		if(results[0] != null){
			var company = results[0].company;
			var usernameDb = results[0].username;
...
app.post('/fenceData', function (req, res) {
	...
	var polyArr = [];
	var coordArr = [];
	if(polyShape.length==0){
		errType = "X";
		errMsg = "No area demarcated";	
	}else{
		connection.query('INSERT INTO fence SET `name`=?,`address`=?,`postal`=?, `type`=?,`lat`=?,`lon`=?,`company`=?,`polygonTest`=PolygonFromText(?)', [areaName,areaAddr,areaPostal,areaType,0,0,coy,"POLYGON((" + polyShape +"))"], function (error, results, fields) {
			if (error) throw error;
				errType = "0";
				errMsg = "Successfully added geofence";
		});
	}
	res.redirect('/demarcate');
});
