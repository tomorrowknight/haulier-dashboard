...
//create app server
var server = app.listen(1983,"dev.logistics.lol", function () {
...
app.get('/vehicleExpenditureTeam/team/:team/', function (req, res) {
	if(req.session.company!=null){
		var company = req.session.company;
		var q = "";
		q = "SELECT AVG(`fuel_consumption`) AS fuel_consumption, AVG(`fuel_remaining`) AS fuel_remaining, AVG(`fuel_price`) AS fuel_price FROM vehicle WHERE `plate_num`=? OR `team`=? OR `company`=? AND company=? ";
		var sqlQuery = connection.query(q,[req.params.team,req.params.team,company,company], function (error, results, fields) {
		if (error) throw error;
			if(results.length){
...
app.post('/uploadData', function (req, res) {
	...
	if(req.session.company!=null){
		...
		var initialURL = '/home/safe/pmf/pmf-dev/pmf-app/';
